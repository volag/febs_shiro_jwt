...
public class QnUploadServiceImpl implements QnUploadService, InitializingBean {
    ...
    public String uploadFile(File file, String fileName) throws QiniuException {
        ...
        if (response.statusCode == 200) {
            return new StringBuffer().append("http://pqggd9642.bkt.clouddn.com/").append(fileName).toString();
        }
        return "上传失败!";
    }
    ...
    @Override
    @Transactional
    public Boolean upload(byte[] localData, String remoteFileName,String key) throws IOException {
//        if(key!=null){
//            delete(key);
//        }
        Response res = uploadManager.put(localData, remoteFileName, getUploadToken());
        if(res.bodyString().equals("")){
            return false;
        }
        return true;
    }
