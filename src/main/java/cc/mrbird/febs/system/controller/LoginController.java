...
public class LoginController {
    ...
    public FebsResponse messageLogin(@NotBlank(message = "{required}") String tel, @NotBlank(message = "{required}") String code,HttpServletRequest request) throws Exception{
        ...
        User user = this.userManager.getUserByMobile(tel);
        String redisCode = userManager.getCode(tel);
        if (user == null)
            throw new FebsException(errorMessage);
        if(!StringUtils.equals(redisCode.replaceAll("\"",""), code)){
            throw new FebsException(errorMessage);
        }
        ...
        String token = FebsUtil.encryptToken(JWTUtil.sign(user.getUsername(), user.getPassword()));
        ...
        Map<String, Object> userInfo = this.generateUserInfo(jwtToken, user,request);
    ...
    public void regist(
            ...
            @NotBlank(message = "{required}") String tel,
            @NotBlank(message = "{required}") String code) throws Exception {
        final String errorMessage = "手机号或验证码错误！";
        String redisCode = userManager.getCode(tel);
        if(!StringUtils.equals(redisCode.replaceAll("\"",""), code)){
            throw new FebsException(errorMessage);
        }
        this.userService.regist(username, password,tel);
    }
...
//        activeUser.setLoginAddress(AddressUtil.getCityInfo(DbSearcher.BTREE_ALGORITHM, ip));
        activeUser.setLoginAddress(AddressUtil.getCityByIp());
     ...
     * @return UserInfo
     */
    private Map<String, Object> generateUserInfo(JWTToken token, User user,HttpServletRequest request) throws IOException {
        String username = user.getUsername();
        Map<String, Object> userInfo = new HashMap<>();
        userInfo.put("token", token.getToken());
        userInfo.put("exipreTime", token.getExipreAt());

        String address = AddressUtil.getCityByIp();
